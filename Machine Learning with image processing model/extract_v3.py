...
def extractLines(img):
    ...
    for i, line in enumerate(fineLines):
        segment = hpList[int(line[0]):int(line[1])] #segment = hpList[int(line[0]):int(line[1])]
        for j, sum in enumerate(segment):
...
def extractSlant(img, words):
    ...
    for i, angle in enumerate(theta):
        ...
        for j, word in enumerate(words):
            ...
            # Resize new_image vertically to match the height of original
            if new_image.shape[0] < original.shape[0]:
                # Extend new_image to have the same height as original
                new_image_resized = np.zeros((original.shape[0], new_image.shape[1]), dtype=new_image.dtype)
                new_image_resized[:new_image.shape[0], :] = new_image
                new_image = new_image_resized
            # new_image[:, pad_length:width+pad_length] = original
            try:
                new_image[:, pad_length:width + pad_length] = original
            except ValueError as e:
                print("Error during assignment:", e)
                print("Ensure the width and pad_length are correctly defined.")
...
def barometer(image):
    ...
    global PEN_PRESSURE
    if image.ndim == 2:
        # Input image has only one channel, so use it directly
        image = image
    else:
...
def start(file_name):
    ...
    image = cv2.imread('F:\\Working Files\\SVM_Graphology\\test_imgs\\'+file_name)
    # cv2.imshow(file_name,image)
    # image = cv2.resize(image, (850, 900))
    barometer(image)
